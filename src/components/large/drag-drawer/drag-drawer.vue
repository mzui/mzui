  ...
  >
    ...
    <template v-for="(slots, slotsName) in $slots">
      <template v-if="slotsName !== 'default'">
        <render-dom v-for="(render, index) in slots" :key="`b_drawer_${slotsName}_${index}`" :render="() => render" :slot="slotsName">
        </render-dom>
      </template>
      <template v-else>
...
<script>
import RenderDom from '@coms/large/libs/render-dom';
import DragDrawerTrigger from './drag-drawer-trigger.vue';
import Mixin from './mixin';
import { on, off } from '@coms/large/libs/tools';
import './index.less';
export default {
  name: 'BDrawer',
  components: {
